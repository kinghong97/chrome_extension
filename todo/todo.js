...
window.onload = () => {
  chrome.storage.local.get(["todo_list"], (result) => {
    const todo_list = JSON.parse(result.todo_list);
    for (const [key, value] of Object.entries(todo_list)) {
      const now_wrapper = document.getElementById(`${key}`);
      for (val of value) {
        if (val[1]) {
          now_wrapper.insertAdjacentHTML(
            "beforeend",
            `
...
function save_all() {
  ...
  chrome.storage.local.set({ todo_list: JSON.stringify(todo_list) });
}
...
function time_check() {
  ...
  chrome.storage.local.get(["time"], (result) => {
    if (result !== {}) {
      const old_time = JSON.parse(result.time);
  ...
  time["date"] = now_date;
  time["day"] = now_day;
  chrome.storage.local.set({ time: JSON.stringify(time) });
}
